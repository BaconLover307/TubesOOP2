//package com.avatarduel.view.cards;
//
//import javafx.scene.text.Font;
//import javafx.scene.text.Text;
//import javafx.scene.layout.*;
//import com.avatarduel.model.cards.card.PowerUp;
//import com.avatarduel.view.cards.SkillDisplay;
//import javafx.scene.text.TextAlignment;
//import javafx.scene.image.Image;
//
//public class PowerUpDisplay extends SkillDisplay {
//
//    private Text atribut;
//    private double attrX, attrY, attrSz, attrW;
//    private double effX, effY, effSz;
//    private String atr;
//    private Text effect;
//
//    public PowerUpDisplay(PowerUp C, Pane pane, double cardW, double cardH, double posX, double posY){
//
//        super(C,pane,cardW,cardH,posX,posY);
//
//        effSz = 0.03 * cardH;
//        effX = 0.1 * cardW;
//        effY = 0.21 * cardH;
//        effect = new Text();
//        effect.setText("POWER UP");
//        effect.setFont(Font.font(java.awt.Font.SERIF, effSz));
//        effect.setX(effX);
//        effect.setY(effY);
//
//        attrX = 0.08 * cardW;
//        attrY = 0.95 * cardH;
//        attrSz = 0.035 * cardH;
//        attrW = 0.85 * cardW;
//        atr = "POW/" + Integer.toString(C.getPowVal());
//        atribut = new Text();
//        atribut.setText(atr);
//        atribut.setFont(Font.font(java.awt.Font.SERIF, attrSz));
//        atribut.setTextAlignment(TextAlignment.RIGHT);
//        atribut.wrappingWidthProperty().set(attrW);
//        atribut.setX(attrX);
//        atribut.setY(attrY);
//
//        super.getBox().getChildren().addAll(atribut,effect);
//    }
//
//
//
//}